...
class Lexer():
    def __init__(self):
        ...
        self.reservedKeywords = ["clear", "incr", "decr",
                                 "copy", "to", "end", "while", "not", "do"]

    # Lexer function
    def tokenise(self, code):
        tokens = []
        tok_regex = "|".join(
            f"(?P<{name}>{pattern})" for name, pattern in self.validTokens)
        # input(tok_regex)
...
class Parser():
    ...
    def aheadChecker(self, ahead, peekAmount, item):
        ...
        if self.currentToken(peekAmount+1)[0] != item:
            ...
            raise SyntaxError(f"expected {item}, got {
                              self.currentToken(peekAmount+1)[0]}")
    ...
    def syntaxAnalysis(self):
        ...
        while self.counter < len(self.tokens):
            ...
            if token[0] == "clear":
                ...
                astList.append(
                    {"type": "clear", "name": self.currentToken(1)[1]})
                self.counter += 2

            elif token[0] == "incr":
                self.checkerGrouper(2, "identifier,semicolon")
                astList.append(
                    {"type": "incr", "name": self.currentToken(1)[1]})
                self.counter += 2

            elif token[0] == "decr":
                self.checkerGrouper(2, "identifier,semicolon")
                astList.append(
                    {"type": "decr", "name": self.currentToken(1)[1]})
                self.counter += 2

            elif token[0] == "copy":
                self.checkerGrouper(4, "identifier,to,identifier,semicolon")
                astList.append({"type": "copy", "from": self.currentToken(1)[
                               1], "to": self.currentToken(3)[1]})
                self.counter += 4

            elif token[0] == "while":
                self.checkerGrouper(5, "identifier,not,zero,do,semicolon")
                astList.append(
                    {"type": "while", "name": self.currentToken(1)[1]})
                self.counter += 5
        ...
        if astList[-1]["type"] != "end":
            raise SyntaxError("barebones code must end with 'end;'")
        return astList
    ...
    def interpret(self, silent=False):
        ...
        while running:
            ...
            if ast["type"] == "clear":
                ...
                self.symbolTable[ast["name"]] = max(
                    self.symbolTable[ast["name"]] - 1, 0)
            elif ast["type"] == "copy":
